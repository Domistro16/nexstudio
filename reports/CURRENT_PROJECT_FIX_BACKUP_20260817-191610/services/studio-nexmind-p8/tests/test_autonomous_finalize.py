...
from orchestrator import run_finalize_p8
...
def judge_ensemble_hash():
    identities=sorted(["fixture-final:final-judge-v1","fixture-audit:perceptual-auditor-v1"])
    return hashlib.sha256(json.dumps(identities,separators=(",",":"),ensure_ascii=False).encode()).hexdigest()
...
class Provider:
    ...
    def audit_dicts(self):
        return [
            {"task":"final_producer","status":"PASS","provider":"fixture-final","resolved_model":"final-judge-v1"},
            {"task":"perceptual_auditor","status":"PASS","provider":"fixture-audit","resolved_model":"perceptual-auditor-v1"},
        ]
