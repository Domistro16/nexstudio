...
export function attestStudioProductionRuntime(req:StudioRuntimeRequirements):StudioRuntimeAttestation{
  ...
  if(fp&&pa)ck("judge-model-independence",fp.model!==pa.model,`${fp.model} != ${pa.model}`);
  const creative=routeFor("NEXMIND_STORY_DIRECTOR","creative_reasoning");if(creative&&fp)ck("creator-final-judge-independence",creative.model!==fp.model,`${creative.model} != ${fp.model}`);if(creative&&pa)ck("creator-auditor-independence",creative.model!==pa.model,`${creative.model} != ${pa.model}`);
