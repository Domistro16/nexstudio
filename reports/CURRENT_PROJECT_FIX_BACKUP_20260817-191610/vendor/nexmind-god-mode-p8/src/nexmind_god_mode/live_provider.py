...
class LiveCreativeModelProvider(CreativeModelProvider):
    def __init__(self, router:Optional[RoleRouter]=None, timeout_s:float=90.0, max_retries:int=2):
        self.router=router or RoleRouter(); self.timeout_s=timeout_s; self.max_retries=max_retries; self.audits=[]; self.candidate_order_audits=[]; self.perceptual_deliveries=[]
    ...
    def _prompt_json_system(self, task:str, route:RoleRoute)->str:
        schema=json.dumps(SCHEMAS[task],ensure_ascii=False,separators=(",",":"))
        return (
    ...
    def _prompt_json_payload(self, task:str, request:Dict[str,Any], route:RoleRoute)->Dict[str,Any]:
        ...
        return {
            "model":route.model,
            "messages":[
                {"role":"system","content":self._prompt_json_system(task,route)},
    ...
    def _validate_schema_node(value:Any, schema:Dict[str,Any], path:str="<root>")->None:
        ...
        maxItems, enum, minimum, and maximum. Keeping this validator local prevents
    ...
    @staticmethod
    def _validate_local_schema(task:str, parsed:Dict[str,Any])->None:
    ...
    def _responses_payload(self, task:str, request:Dict[str,Any], route:RoleRoute)->Dict[str,Any]:
        ...
        return {
            ...
            "text":{"format":{"type":"json_schema","name":self._schema_name(task),"schema":SCHEMAS[task],"strict":True}},
            "store":False,
        }
    ...
    def _chat_payload(self, task:str, request:Dict[str,Any], route:RoleRoute)->Dict[str,Any]:
        ...
        return {
            ...
            "response_format":{"type":"json_schema","json_schema":{"name":self._schema_name(task),"schema":SCHEMAS[task],"strict":True}},
        ...
        for attempt in range(self.max_retries+1):
            try:
                ...
                if route.api_mode=="chat_completions_prompt_json":
                    parsed=self._strip_schema_only_extras(parsed,SCHEMAS.get(task) or {})
                    self._validate_local_schema(task,parsed)
                inp,cached,out,reas=self._usage(data)
                audit=ProviderCallAudit(task,route.role,route.provider,route.provider,route.model,resolved,route.reasoning,req_hash,canonical_hash(parsed),rid,inp,cached,out,reas,int((time.monotonic()-started)*1000),retries,"PASS")
