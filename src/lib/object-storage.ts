...
function remoteConfig(): RemoteConfig | null {
  ...
  if (process.env.NODE_ENV === "production" && !env.objectStorageAllowLocalProduction) throw new Error("S3-compatible object storage must be configured in production unless explicit single-node local storage is enabled.");
  return null;
}
